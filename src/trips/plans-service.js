...
const PlansService = {
	getAllPlans(db) {
		return db
			...
			.select(
				...
				'pd.from_formatted_address',
				'pd.from_international_phone_number',
				'pd.from_website',
				'pd.from_terminal',
				'pd.from_gate',
				'pd.to_name',
				...
				'pd.to_formatted_address',
				'pd.to_international_phone_number',
				'pd.to_website',
				'pd.to_terminal',
				'pd.to_gate'
			)
	...
	serializePlan(plan) {
		...
		return {
			...
			formatted_address: xss(planData.formatted_address),
			international_phone_number: xss(planData.international_phone_number),
			website: xss(planData.website),
			plan_detail_id: planData.plan_detail_id,
			plan_subtype: planData.plan_subtype,
			from_name: xss(planData.from_name),
			from_place_id: planData.from_place_id,
			from_utc_offset_minutes: planData.from_utc_offset_minutes,
			from_formatted_address: xss(planData.from_formatted_address),
			from_terminal: xss(planData.from_terminal),
			from_gate: xss(planData.from_gate),
			from_international_phone_number: xss(planData.from_international_phone_number),
			from_website: xss(planData.from_website),
			to_name: xss(planData.to_name),
			to_place_id: planData.to_place_id,
			to_utc_offset_minutes: planData.to_utc_offset_minutes,
			to_formatted_address: xss(planData.to_formatted_address),
			to_international_phone_number: xss(planData.to_international_phone_number),
			to_website: xss(planData.to_website),
			to_terminal: xss(planData.to_terminal),
			to_gate: xss(planData.to_gate),
			comparable_date
		};
	}
}
